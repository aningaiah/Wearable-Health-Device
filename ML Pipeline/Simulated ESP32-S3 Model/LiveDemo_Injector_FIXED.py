...
# Heartbeat: re-send the current state every 3 s so the Arduino knows
# the connection is still alive and doesn't revert to standalone mode
...
def update(frame):
    ...
    global _alert_active, _last_heartbeat
    ...
    if is_alert:
        ...
        if not _alert_active:
            ...
            _alert_active = True
            _last_heartbeat = time.monotonic()   # reset so heartbeat doesn't fire immediately
    else:
    ...
    # Heartbeat — re-send the current state every 3 s so the Arduino never
    # times out and reverts to its standalone sensor screen
    now_t = time.monotonic()
    if now_t - _last_heartbeat >= HEARTBEAT_INTERVAL:
        send_display_command(alert=_alert_active)
        _last_heartbeat = now_t
