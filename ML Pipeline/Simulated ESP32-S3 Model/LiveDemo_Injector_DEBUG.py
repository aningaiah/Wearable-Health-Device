...
def read_serial():
    ...
    if ser and ser.in_waiting > 0:
        try:
            ...
            # ── DEBUG: print every raw line received from the ESP32 ──────────
            # Shows exactly what COM5 is sending so you can diagnose why
            # the plots aren't updating.  Comment this block out once fixed.
            if not hasattr(read_serial, '_debug_count'):
                read_serial._debug_count = 0
            if read_serial._debug_count < 50:          # print first 50 lines only
                print(f"[DEBUG RAW #{read_serial._debug_count:03d}] '{line}'  "
                      f"(fields={len(line.split(','))}, "
                      f"in_waiting={ser.in_waiting})")
                read_serial._debug_count += 1
            elif read_serial._debug_count == 50:
                print("[DEBUG] First 50 lines captured — suppressing further raw output.")
                read_serial._debug_count += 1
            # ── END DEBUG ────────────────────────────────────────────────────

            # Skip the CSV header line the sketch emits on startup
            ...
            if len(d) == 9:
                ...
                # ── DEBUG: flag lines that connect but have wrong field count
                print(f"[DEBUG FIELD MISMATCH] Expected 9 fields, got "
                      f"{len(d)}: '{line}'")
        except (ValueError, Exception) as e:
            # ── DEBUG: surface silent parse errors
            print(f"[DEBUG EXCEPTION] {type(e).__name__}: {e}  |  raw='{line if 'line' in dir() else 'unread'}'")
            pass
